...
def UCS_search(node, goal):
    ...
    open.append(node)
    while open:                         # we only want this search to stop, when the cost of the items in the queue are all greater than our lowest goal state
        if queueCost(open) >= goalNode.cost:
            print(','.join(trackingPath(node)))
            print("There were %d expansions" % expansion)
            return
        ...
        for child in node.children:                                 # expanding the node to the queue as node objects
            if tuple(child[0]) not in visited:                      # eliminates redundancy
                open.append(Node(child[0], node, None, child[1], node.cost + 1))
        for item in open:                                          # searches the queue until it finds a node with the least cost
            if item.cost == queueCost(open):
                open.remove(item)
                visited.add(tuple(item.state))
                node = item
                break
    print(','.join(trackingPath(node)))
    print("There were %d expansions" % expansion)
