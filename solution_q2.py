...
def DFS_Search(node):
    ...
    while (int(node.state[0])+int(node.state[1])+int(node.state[2]))!=11:                                       # will run until node is at goal state
        foundAlready = False
...
def BFS(start_node):
    ...
    while open:
        ...
        if (int(current_state[0])+int(current_state[1])+int(current_state[2]))==11:
    ...
    while queueCost(queue) < goalNode.cost:                         # we only want this search to stop, when the cost of the items in the queue are all greater than our lowest goal state
        ...
        for item in queue:                                          # searches the queue until it finds a node with the least cost
            if item.cost == queueCost(queue):
                ...
                node = item
                break
        if (int(node.state[0])+int(node.state[1])+int(node.state[2]))==11 and node.cost < goalNode.cost:        # if we find a goal node of less cost, we will change the current goalNode
            goalNode = node
    print(','.join(trackingPath(node)))
